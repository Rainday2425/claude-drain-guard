...
function historyBars(items) {
  ...
  return items.map(item => `<span class="bar ${escapeHtml(item.risk)}" style="height:${barHeight(item.fresh, peak)}%" data-tooltip="${escapeHtml(historyTooltip(item))}" aria-label="${escapeHtml(historyTooltip(item).replace(/\n/g, ', '))}"></span>`).join('');
}
...
function sessionBars(session) {
  ...
  return session.turns.map((turn, index) => {
    ...
    return `<span class="session-bar ${escapeHtml(turn.risk)}" style="height:${barHeight(Number(turn.fresh) || 0, peak)}%" data-tooltip="${escapeHtml(label)}" aria-label="${escapeHtml(label.replace(/\n/g, ', '))}"></span>`;
  }).join('');
}
